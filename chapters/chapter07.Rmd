...
An understanding of basic operations to be used with your statistical software is a good place to start.  In `R`, logical operators include `!` (not), `&` and `&&` (logical AND), `|` and `||` (logical OR).  Relational operators in `R` include `<` (less than), `>` (greater than), `<=` (thes than or equal), `==` (vector equality), and `!=` (not equal).
